"""
Genre Resolver for Abridge Pipeline (Tier-3.4a)

PURPOSE:
This module resolves GENRE SIGNALS from upstream Tier-3 data using deterministic,
rule-based evaluation. It aggregates evidence from character salience, relationship
patterns, and event keywords to produce confidence-scored genre assignments.

============================================================
WHY GENRE DETECTION EXISTS
============================================================

Genres frame reader expectations. They answer:
"What kind of story is this?"

Genre detection in Abridge is DESCRIPTIVE, not EVALUATIVE:
- It does NOT judge literary quality
- It does NOT interpret authorial intent
- It does NOT collapse to a single "best" genre
- It DOES report which genres have observable evidence
...
- Consumes ONLY Tier-3.1 (Salience), Tier-3.2 (Relationships), Tier-3.3 (Keywords)
- Applies deterministic rules to compute genre confidence
- Produces multi-genre output with evidence attribution
...
- Match text against genre descriptions
- Interpret narrative meaning
- Force genre assignment when evidence is weak
- Produce a single "primary" genre
- Judge story quality
...
Genres are drawn from Feydar/novelupdates_genre taxonomy.
Only genres with implementable rules are included.
Genre descriptions are for documentation only — NEVER used as classifiers.
...
- Multiple genres can have high confidence (multi-genre novels)
- Low confidence means "insufficient evidence", not "definitely not this genre"
- Empty results mean "no genres have sufficient evidence"
...
Genre data is stored per run_id in:
- JSON artifact: data/genre_resolved/{novel_name}/{run_id}.genre_resolved.json
...
from dict.genre_dictionary import GENRE_TAXONOMY_VERSION, GENRE_TAXONOMY, GENRE_RULE_VERSION, GENRE_RULES
...
@dataclass
class GenreEvidence:
    """Evidence supporting a genre assignment."""
...
@dataclass
class GenreResult:
    """Result for a single genre resolution."""
    genre_id: str
    display_name: str
    confidence: float
...
@dataclass
class GenreResolvedMap:
    """Complete genre resolution output for a novel."""
    novel_name: str
    run_id: str
    tier: str = "tier-3.4a"
    taxonomy_version: str = GENRE_TAXONOMY_VERSION
    rule_version: str = GENRE_RULE_VERSION
    confidence_threshold: float = CONFIDENCE_THRESHOLD
    total_genres_evaluated: int = 0
    genres_above_threshold: int = 0
...
class GenreRuleEngine:
    """
    Deterministic rule evaluation engine for genre resolution.
    ...
    def evaluate_genre(self, genre_id: str, rule: dict) -> GenreResult:
        ...
        Returns GenreResult with confidence score and evidence.
        """
        evidence = GenreEvidence()
        ...
        if not required_met:
            ...
            return GenreResult(
                genre_id=genre_id,
                display_name=GENRE_TAXONOMY.get(genre_id, {}).get("display_name", genre_id),
                confidence=0.0,
                evidence=evidence,
                required_met=False,
...
def build_genre_resolved_map(
    ...
    2. Evaluates each genre against its rule
    ...
    for genre_id, rule in GENRE_RULES.items():
        genre_result = engine.evaluate_genre(genre_id, rule)
        genre_results.append(genre_result)

    result.total_genres_evaluated = len(genre_results)

    # Filter by confidence threshold and add to output
    for gr in genre_results:
        if gr.confidence >= CONFIDENCE_THRESHOLD:
            result.genres[gr.genre_id] = {
                "display_name": gr.display_name,
                "confidence": gr.confidence,
                "evidence": {
                    "event_keywords": gr.evidence.event_keywords,
                    "event_categories": gr.evidence.event_categories,
                    "keyword_spreads": gr.evidence.keyword_spreads,
                    "keyword_densities": gr.evidence.keyword_densities,
                ...
                "scoring": {
                    ...
                    "boosts_applied": gr.boosts_applied,
                    "penalties_applied": gr.penalties_applied,
                },
            }

    result.genres_above_threshold = len(result.genres)

    return result


def generate_genre_resolved(
        novel_name: str,
        run_id: str,
) -> Optional[str]:
    """
    ...
    """
    print(f"\n[Genre Resolver] Resolving genres from Tier-3 evidence...")
    print(f"[Genre Resolver] Taxonomy version: {GENRE_TAXONOMY_VERSION}")
    print(f"[Genre Resolver] Rule version: {GENRE_RULE_VERSION}")
    print(f"[Genre Resolver] Confidence threshold: {CONFIDENCE_THRESHOLD}")
    ...
    # Log artifact status
    if event_keywords:
        print(f"[Genre Resolver] Loaded event keywords: {event_keywords.get('total_keywords_found', 0)} keywords")
    else:
        print(f"[Genre Resolver] WARNING: No event keywords artifact found")

    if character_salience:
        print(f"[Genre Resolver] Loaded character salience: {len(character_salience.get('characters', []))} characters")
    else:
        print(f"[Genre Resolver] WARNING: No character salience artifact found")

    if relationship_matrix:
        print(f"[Genre Resolver] Loaded relationship matrix: {len(relationship_matrix.get('pairs', {}))} pairs")
    else:
    ...
    # Report results
    print(f"[Genre Resolver] Evaluated {result.total_genres_evaluated} genres")
    print(f"[Genre Resolver] Found {result.genres_above_threshold} genres above threshold")
    ...
    if result.genres:
        ...
        print(f"[Genre Resolver] Top genres:")
        for genre_id, data in sorted_genres[:5]:
            evidence_summary = []
        ...
        print(f"[Genre Resolver] No genres met the confidence threshold")

    # Persist artifact
    output_dir = os.path.join(GENRE_RESOLVED_DIR, novel_name)
    os.makedirs(output_dir, exist_ok=True)

    output_path = os.path.join(output_dir, f"{run_id}.genre_resolved.json")
    ...
    output_data = {
        ...
        "total_genres_evaluated": result.total_genres_evaluated,
        "genres_above_threshold": result.genres_above_threshold,
    ...
    print(f"[Genre Resolver] Saved to: {output_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python genre_resolver.py <novel_name> [run_id]")
        print("  If run_id is omitted, uses 'standalone_genre' as identifier")
        sys.exit(1)

    novel_name = sys.argv[1]
    run_id = sys.argv[2] if len(sys.argv) > 2 else "standalone_genre"

    result_path = generate_genre_resolved(novel_name, run_id)

    if result_path:
        print(f"\n✓ Genre resolution generated: {result_path}")
    else:
        print(f"\n✗ Genre resolution failed")
        sys.exit(1)
